...
@click.group(name="keys", short_help="key formatter CLI")
@click.option(
...
def keys(ctx: Context, debug: bool, keys_dir: Optional[str]) -> None:
    ...
    definitions of keybinds that are then serialized to JSON in a standard format
...
def keys_format(formatter: str, sequence: Tuple[KeySequence, ...]) -> None:
    ...
    Given a formatter and a keys key sequence(s), this will output the sequence in
...
def keys_send(fzf: bool, sender: str, sequence: Tuple[str, ...]) -> None:
    ...
    Given a sender and key sequence(s), this will format and send the key sequence
...
def keys_get(ctx: Context, app: str, group: str, binds: Tuple[str, ...]) -> None:
    """retrieve a keybind(s) for an application

    Given an app, group, and keybind name(s), this will retrieve the keybind(s)
...
@click.option(
    ...
    "_keys",
    type=str,
    help="a file or import path to root tome, defaults to ~/.config/keyslib/keys.py",
    envvar="KEYS_FILE",
)
@pass_context
def keys_build(ctx: Context, _keys: Optional[str]) -> None:
    ...
    This will build all <app>.py files under the keys dir and write all registered
    keybinds to their respective <app>.env files. This file can then be parsed
    like any other dotenv file or using tools like aeconfig using aecode.lib.keys
    in order to generate configuration files with the desired mappings.
    """
